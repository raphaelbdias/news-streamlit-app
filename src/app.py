...
def fetch_sources():
    ...
    if r.status_code == 200:
        return sorted([(s["id"], s["name"]) for s in r.json()["sources"]], key=lambda x: x[1])
    else:
        return []

def fetch_news(country, category, query, sources):
...
def display_news(articles):
    for art in articles:
        # parse & format publishedAt if available
        pub = art.get("publishedAt")
        if pub:
            try:
                # convert ISO timestamp into local datetime
...
def main():
    ...
    country  = st.sidebar.selectbox("Country",    ["us","gb","ca","au","in","de","fr","it","jp"], index=0)
    category = st.sidebar.selectbox("Category",   ["business","entertainment","general","health","science","sports","technology"], index=6)
    query    = st.sidebar.text_input("Keyword search")

    all_sources = fetch_sources()
    sources = st.sidebar.multiselect(
        "Sources (overrides country & category)",
    ...
    if "articles" not in st.session_state:
        st.session_state.articles = fetch_news(country, category, query, sources)
